...
class CustomSend extends StatefulWidget {
  ...
  /// CustomSendController to help control the animation progress.
  /// Call [controller.completed()] when getting response from your request
  /// Call [controller.reset()] to restart button animation
  final CustomSendController controller;

  /// Customize on pressing action
  final VoidCallback onPressed;

  @override
  _CustomSendState createState() => _CustomSendState();
}

class _CustomSendState extends State<CustomSend> with TickerProviderStateMixin {
                    ...
                    Center(
                      child: Text(
                        ...
                        style: text22.copyWith(
                          color: Colors.white,
                          fontSize: screenWidth(context) > 700 ? 30.sp : 80.sp,
                        ),
                      ),
                    ),
                  ],
...
class CustomSendController extends ValueNotifier<bool> {
  OnLoaded _callback;

  CustomSendController({bool value = false}) : super(value);
